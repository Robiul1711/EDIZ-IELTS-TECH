  ...
  return (
    <div className=" section-padding-x py-10 text-slate-700">
      <div className="space-y-6">
        ...
        {/* Header Section */}
        <header className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 p-6 shadow-sm">
          <h1 className="text-xl font-medium text-slate-600 dark:text-white">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <article className="lg:col-span-6 bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm overflow-hidden flex flex-col h-full min-h-[600px]">
            <div className="p-10 overflow-y-auto relative custom-scrollbar">
              <div className="text-center mb-10">
                <h2 className="text-xl font-bold text-slate-800 dark:text-white mb-1">The Greywater Bridge Crisis</h2>
                <p className="italic text-slate-500 dark:text-slate-400 text-sm">What Would You Do If a Bridge You Designed Had a Fatal Flaw?</p>
              </div>

              <div className="space-y-8 text-[15px] leading-relaxed text-slate-600">
                <section>
                  <span className="text-indigo-600 dark:text-indigo-400 font-bold block mb-2 text-lg">1.</span>
          ...
          <div className="lg:col-span-6 space-y-6">
            ...
            {/* Instructions Box */}
            <div className="bg-[#F5F3FF] dark:bg-slate-800 rounded-2xl p-6 border border-indigo-50 dark:border-slate-700">
              <h3 className="font-bold text-slate-800 dark:text-white mb-2 text-lg">Questions 01–05</h3>
            ...
            <div className="space-y-4 ">
              {questions.map((q, idx) => (
                <div key={idx} className="space-y-4 ">
                  <div className={`p-4 rounded-xl transition-all dark:bg-slate-800  ${idx === 0 ? 'bg-indigo-50  border border-indigo-100 dark:border-slate-700' : 'bg-[#F1F3F5]'}`}>
                    <p className="text-sm font-medium text-slate-700 dark:text-white">
                      <span className="text-indigo-600 dark:text-indigo-400 mr-2">{idx + 1}.</span> {q}
                    </p>
                  </div>
                        ...
                        <label key={option} className="flex items-center space-x-3 cursor-pointer group ">
                          <div className="relative flex items-center justify-center">
                            <input
                              type="radio"
                              name="bridge-q"
                              className="peer appearance-none w-5 h-5 border-2 border-slate-400 dark:border-slate-700 rounded-full checked:border-indigo-600 transition-all"
                          ...
                          <span className="text-xs font-bold text-slate-500 dark:text-slate-400 group-hover:text-slate-700 transition-colors uppercase tracking-wide">
                            {option}
                          </span>
                        </label>
                      ))}
