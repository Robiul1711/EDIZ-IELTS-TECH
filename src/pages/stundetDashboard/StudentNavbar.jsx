  ...
  return (
    <nav className="relative flex items-center justify-between pt-8 bg-transparent">
      <div className="flex items-center gap-4">
        ...
        <button 
          onClick={onMenuClick}
          className="xmd:hidden p-2 rounded-xl bg-white border border-slate-200 text-[#604CDF] shadow-sm active:scale-95 transition-transform"
        >
          <Menu size={24} />
        </button>
      ...
      <div className="flex items-center gap-3 md:gap-6">
        ...
        >
          <div className={`absolute z-10 flex h-6 w-6 items-center justify-center rounded-full bg-white shadow-md transition-transform duration-300 ${isDarkMode ? 'translate-x-6 md:translate-x-8' : 'translate-x-0'}`}>
            {isDarkMode ? <Moon size={12} className="text-[#5E4FD7]" /> : <Sun size={12} className="text-[#5E4FD7]" />}
          </div>
        ...
        <div className="relative" ref={dropdownRef}>
          <button onClick={() => setIsOpen(!isOpen)} className="flex h-10 w-10 md:h-11 md:w-11 items-center justify-center rounded-full border-2 border-[#5E4FD7] bg-white">
          ...
          {isOpen && (
            <div className="absolute right-0 mt-4 w-56 md:w-64 origin-top-right rounded-2xl bg-white py-2 shadow-2xl z-50 ring-1 ring-black/5">
              <button className="flex w-full items-center gap-4 px-5 py-3 text-slate-600 hover:bg-slate-50 transition-colors">
                <User size={18} strokeWidth={1.5} /> <span className="font-medium">Profile</span>
              </button>
              ...
              <button className="flex w-full items-center gap-4 px-5 py-3 text-red-500 hover:bg-red-50 transition-colors">
                <LogOut size={18} strokeWidth={1.5} /> <span className="font-medium">Log out</span>
              </button>
            </div>
          )}
        </div>
