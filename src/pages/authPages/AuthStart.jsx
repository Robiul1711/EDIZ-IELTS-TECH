import React from "react";
import { Link } from "react-router-dom";

const AuthStart = () => {
  return (
    /* Main Card Container */
    <div className="w-full max-w-sm bg-white rounded-[32px] overflow-hidden shadow-2xl">
      {/* Header Section */}
      <div className="bg-[#604CDF] p-8 pb-10">
        <p className="text-white/90 text-sm mb-2 font-medium">
          No Payment Required
        </p>
        <h1 className="text-white text-2xl font-bold leading-tight">
          Sign Up And Get One Free Mock Test
        </h1>
      </div>

      {/* Body Section */}
      <div className="p-8 pt-6">
        <h2 className="text-[#5D5FEF] text-lg font-bold mb-6">Login/Sign Up</h2>

        <div className="space-y-4">
          {/* Google Button */}
          <button className="group relative w-full flex items-center justify-center bg-gradient-to-r from-[#AC50EF] to-[#705CF6] py-3 rounded-full shadow-lg hover:shadow-purple-200  transition-transform active:scale-[0.98]">
            <div className="absolute left-2 bg-white rounded-full p-1.5 shadow-sm">
              <svg className="w-5 h-5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
              </svg>
            </div>
            <span className="text-white font-semibold text-sm pl-6">
              Start with Google
            </span>
          </button>

          {/* Email Button (Changed from button to Link for proper navigation) */}
          <Link 
            to="/auth/login" 
            className="group relative w-full flex items-center justify-center bg-white border border-gray-200 py-3 rounded-full hover:bg-gray-50 transition-colors active:bg-gray-100"
          >
            <div className="absolute left-2 bg-[#E2E4E9] rounded-full p-2">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 text-gray-500">
                <path d="M1.5 8.67v8.58a3 3 0 003 3h15a3 3 0 003-3V8.67l-8.928 5.493a3 3 0 01-3.144 0L1.5 8.67z" />
                <path d="M22.5 6.908V6.75a3 3 0 00-3-3h-15a3 3 0 00-3 3v.158l9.714 5.978a1.5 1.5 0 001.572 0L22.5 6.908z" />
              </svg>
            </div>
            <span className="text-gray-600 font-semibold text-sm pl-6">
              Start with Email
            </span>
          </Link>
        </div>
      </div>
    </div>
  );
};

export default AuthStart;