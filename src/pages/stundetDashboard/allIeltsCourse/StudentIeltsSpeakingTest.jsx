    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-slate-50 dark:bg-slate-950">
        <div className="flex flex-col items-center">
            ...
            <p className="mt-4 text-slate-500 font-bold text-xs uppercase tracking-widest animate-pulse">Initializing Lab...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-screen bg-white dark:bg-slate-950 overflow-hidden font-sans">
      <TestHeader
        ...
        onExit="/student-dashboard/ielts/speaking"
      />
           ...
           <div className="max-w-2xl mx-auto space-y-10 pb-20">
              ...
              {part_no === "2" && testPart?.cue_card && (
                <div className="p-8 bg-indigo-600 text-white rounded-[3rem] shadow-2xl space-y-4 relative overflow-hidden group">
                    ...
                    <h3 className="text-xs font-black uppercase tracking-widest text-indigo-200 border-b border-white/10 pb-3">Cue Card</h3>
                    <div 
              ...
              <div className="space-y-8">
                 {questions.map((q, idx) => (
                    <div key={idx} className="bg-slate-50 dark:bg-slate-800/50 p-8 rounded-[2rem] border border-slate-100 dark:border-slate-800 transition-all hover:shadow-md group">
        ...
        <div className="flex-1 h-full flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950 p-10 relative overflow-hidden">
             ...
             <div className="relative z-10 w-full max-w-md text-center space-y-12">
                <div className="space-y-4">
                    <h2 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Record Your Answer</h2>
                    <p className="text-slate-500 text-sm font-medium">Please speak clearly into your microphone.</p>
                </div>
                    ...
                    >
                        {isRecording ? (
                            <>
                                ...
                                <span className="mt-3 text-[10px] font-black uppercase tracking-widest">Stop Recording</span>
                            </>
                        ) : (
                            <>
                                <Mic size={48} className="group-hover:animate-bounce" />
                                <span className="mt-3 text-[10px] font-black uppercase tracking-widest">Start Recording</span>
                            </>
                        )}
                    </button>
                </div>
                ...
                <div className="flex flex-col items-center gap-4">
                    <div className="flex items-center gap-3 px-6 py-3 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800">
                        <Clock size={16} className={isRecording ? 'text-red-500' : 'text-indigo-600'} />
                        <span className={`text-xl font-mono font-bold tracking-widest ${isRecording ? 'text-red-500' : 'text-slate-700 dark:text-slate-200'}`}>
                    ...
                    {audioUrl && !isRecording && (
                         <div className="w-full  p-2 bg-indigo-50 dark:bg-indigo-900/20 rounded-3xl border border-indigo-100 dark:border-indigo-800/40 flex items-center justify-between">
                            ...
                            <button 
                                onClick={() => { setAudioBlob(null); setAudioUrl(null); setRecordingDuration(0); }}
      ...
      <footer className="h-24 bg-white dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800 flex items-center px-6 lg:px-12 z-50">
          ...
          >
            <span>{isSubmitting ? "Uploading Audio..." : isSubmitted ? "Submitted Successfully" : "Submit Speaking Test"}</span>
            ...
            {isSubmitted && <CheckCircle2 size={22} className="text-emerald-400 ml-2" />}
          </button>
      </footer>
