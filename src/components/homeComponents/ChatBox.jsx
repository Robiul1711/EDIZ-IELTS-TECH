import React, { useState, useEffect, useRef } from "react";
import { X, Edit2, History } from "lucide-react";
import { ChatBot } from "../svg/AllSVG";
import { useApiQuery } from "@/hooks/apiQuery";
import { useApiMutation } from "@/hooks/apiMutation";

const ChatBox = ({ onClose }) => {
  const [showHistory, setShowHistory] = useState(false);
  const [messages, setMessages] = useState([]);
  const [prompt, setPrompt] = useState("");
  const messagesEndRef = useRef(null);

  const { data: historyData, isLoading: isHistoryLoading } = useApiQuery({
    queryKey: ["gemini-history"],
    url: "/gemini/history",
    secure: true,
  });

  const { mutate: askMutate, isPending: isAskPending } = useApiMutation({
    url: "/gemini/ask",
    method: "POST",
    secure: true,
    toast: false,
    successMessage: "Response generated",
    invalidateKeys: ["gemini-history"],
    onSuccess: (response) => {
      const assistantResponse =
        response?.data?.response || response?.response || "No response from AI.";
      setMessages((prev) => [
        ...prev,
        { id: Date.now(), role: "assistant", content: assistantResponse },
      ]);
    },
  });

  const onSubmit = (event) => {
    event.preventDefault();

    const promptValue = prompt?.trim();
    if (!promptValue) return;

    setMessages((prev) => [
      ...prev,
      { id: Date.now(), role: "user", content: promptValue },
    ]);

    askMutate({ prompt: promptValue });
    setPrompt("");
  };

  useEffect(() => {
    if (historyData?.data?.length) {
      const normalizedMessages = historyData.data.map((item) => ({
        id: item.id,
        role: item.role,
        content: item.content,
      }));
      setMessages(normalizedMessages);
    }
  }, [historyData]);

  useEffect(() => {
    if (messagesEndRef.current) {
      messagesEndRef.current.scrollIntoView({ behavior: "smooth" });
    }
  }, [messages]);
  return (
    <div className="w-[calc(100vw-px)] sm:w-[350px] md:w-[400px] h-[450px] sm:h-[550px] max-h-[80vh] bg-white dark:bg-slate-900 rounded-3xl shadow-2xl overflow-hidden flex flex-col border border-purple-100 dark:border-slate-800 animate-in fade-in slide-in-from-bottom-10 duration-300">
      {/* Header */}
      <div className="bg-purple-50/80 dark:bg-gray-800 p-4 flex justify-between items-start h-[70px] relative">
        <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-[#EAE6FF] to-transparent opacity-50"></div>
        <div className="bg-[#604CDF] p-2 rounded-full z-10 w-fit shadow-md">
          <div className="w-6 h-6 flex items-center justify-center text-white">
            <ChatBot className="w-6 h-6" />
          </div>
        </div>
        <div className="flex gap-2 z-10">
          {/* <button
            onClick={() => setShowHistory(prev => !prev)}
            className="text-gray-500 dark:text-slate-400 dark:hover:text-slate-200 hover:text-gray-700 cursor-pointer"
            aria-label="Toggle chat history"
          >
            <History className="w-5 h-5" />
          </button>
          <button className="text-gray-500 dark:text-slate-400 dark:hover:text-slate-200 hover:text-gray-700 cursor-pointer">
            <Edit2 className="w-5 h-5" />
          </button> */}
          <button
            onClick={onClose}
            className="text-gray-500 dark:text-slate-400 dark:hover:text-slate-200 hover:text-gray-700 cursor-pointer"
            aria-label="Close chat"
          >
            <X className="w-6 h-6" />
          </button>
        </div>
      </div>

      {/* Body */}
      <div
        className="flex-1 p-4 md:p-6 overflow-y-auto space-y-6 bg-white dark:bg-slate-900 relative"
        style={{
          scrollbarWidth: "none",
          msOverflowStyle: "none",
        }}
      >
        {showHistory && (
          <div className="absolute inset-x-4 top-4 z-20 bg-white dark:bg-slate-800 border border-purple-100 dark:border-slate-700 rounded-2xl p-3 shadow-lg text-sm">
            <div className="flex items-center justify-between mb-2">
              <span className="font-semibold text-gray-700 dark:text-slate-200">History</span>
              <button
                onClick={() => setShowHistory(false)}
                className="text-gray-500 hover:text-gray-700 dark:text-slate-400 dark:hover:text-slate-200"
                aria-label="Close history"
              >
                ✕
              </button>
            </div>
            <ul className="space-y-1 max-h-40 overflow-y-auto">
              {historyData?.data?.length > 0 ? (
                historyData.data.map((item, idx) => (
                  <li
                    key={item.id || idx}
                    className="px-2 py-1 rounded-lg hover:bg-purple-50 dark:hover:bg-slate-700 cursor-pointer text-gray-600 line-clamp-1 dark:text-slate-300 whitespace-pre-wrap break-words"
                  >
                    {item.content || "(no content)"}
                  </li>
                ))
              ) : (
                <li className="px-2 py-1 text-gray-400 dark:text-gray-500">No chat history yet</li>
              )}
            </ul>
          </div>
        )}
        {messages && messages.length > 0 ? (
          messages.map((message) => (
            <div
              key={message.id}
              className={`flex ${
                message.role === "user" ? "justify-end" : "justify-start"
              }`}
            >
              <div
                className={`p-4 rounded-3xl max-w-[90%] text-[15px] leading-relaxed font-medium shadow-sm ${
                  message.role === "user"
                    ? "bg-[#F3F4F6] dark:bg-slate-800 text-gray-700 dark:text-slate-200 rounded-tr-sm"
                    : "bg-purple-600 dark:bg-purple-700 text-white rounded-tl-sm"
                }`}
              >
                <pre className="whitespace-pre-wrap break-words m-0">{message.content}</pre>
              </div>
            </div>
          ))
        ) : (
          <div className="text-center text-gray-500 dark:text-slate-400 text-sm mt-4">
            Start the conversation by asking a question above.
          </div>
        )}

        {isAskPending && (
          <div className="flex items-center gap-3 text-gray-500 dark:text-slate-300">
            <div className="bg-[#604CDF] p-2 rounded-full w-10 h-10 flex items-center justify-center shadow-sm shrink-0">
              <div className="scale-75 text-white">
                <ChatBot />
              </div>
            </div>
            <span className="text-sm font-medium animate-pulse">Thinking...</span>
          </div>
        )}
        <div ref={messagesEndRef} />
      </div>

      {/* Footer / Input */}
      <div className="p-4 bg-white dark:bg-slate-900">
        <form onSubmit={onSubmit} className="relative group">
          <textarea
            value={prompt}
            onChange={(e) => setPrompt(e.target.value)}
            onKeyDown={(e) => {
              if (e.key === "Enter" && !e.shiftKey) {
                e.preventDefault();
                onSubmit(e);
              }
            }}
            placeholder="Ask anything........"
            className="w-full h-[120px] bg-gradient-to-br from-[#F5F3FF] to-[#FDFDFF] dark:from-slate-800 dark:to-slate-900 rounded-[2rem] border border-purple-100 dark:border-slate-700 p-6 pr-14 focus:outline-none focus:ring-2 focus:ring-purple-200 dark:focus:ring-slate-700 focus:border-purple-300 dark:focus:border-slate-600 resize-none text-gray-700 dark:text-slate-200 placeholder-gray-400 text-base shadow-inner transition-all"
          ></textarea>
          <button
            type="submit"
            disabled={!prompt.trim() || isAskPending}
            className="absolute bottom-5 right-5 bg-white dark:bg-slate-800 p-2 rounded-full shadow-md hover:shadow-lg transition-all cursor-pointer border border-purple-50 dark:border-slate-700"
          >
            {isAskPending ? (
              <div className="w-5 h-5 border-2 border-purple-200 dark:border-slate-700 border-t-purple-600 dark:border-t-purple-500 rounded-full animate-spin"></div>
            ) : (
              <div className="w-5 h-5 flex items-center justify-center">
                ➤
              </div>
            )}
          </button>
        </form>
      </div>
    </div>
  );
};

export default ChatBox;
