...
const ReadingReview = () => {
  ...
  const questions = [
    { id: 1, text: "The original design of the Greywater Bridge included high-density cable joints.", status: 'incorrect', userAnswer: 'True', correctAnswer: 'False' },
    { id: 2, text: "The original design of the Greywater Bridge included high-density cable joints.", status: 'neutral' },
    { id: 3, text: "The original design of the Greywater Bridge included high-density cable joints.", status: 'neutral' },
    { id: 4, text: "The original design of the Greywater Bridge included high-density cable joints.", status: 'neutral' },
  ];

  return (
    <div className="min-h-screen bg-[#FBFBFF] p-6 rounded-xl  font-sans relative overflow-hidden">
        <div className='mb-6'>
             <PaginationSection/>
        </div>
      <div className={`flex gap-8 transition-all duration-500 ${isFeedbackOpen ? 'mr-[400px]' : ''}`}>
        ...
        <div className="w-64 space-y-4 shrink-0">
          <div className="bg-[#C2410C] rounded-xl overflow-hidden shadow-sm">
            ...
            <div className="bg-white m-1.5 rounded-lg p-3 border border-slate-100">
              <span className="text-slate-700 font-medium">True</span>
            </div>
          </div>
        ...
        <div className="flex-1 space-y-6">
          <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100">
            <h2 className="text-xl font-bold text-slate-800 mb-4">Identifying Information</h2>
              ...
              <div 
                ...
                className={`group cursor-pointer rounded-2xl p-5 border transition-all duration-200 ${
                  selectedQuestion === q.id 
                    ? 'bg-[#E0E7FF]/40 border-[#6C5CE7] shadow-md' 
                    : 'bg-white border-slate-100 hover:border-slate-300 shadow-sm'
                }`}
              >
                <div className="flex items-start gap-4">
                  <span className={`font-bold ${selectedQuestion === q.id ? 'text-[#6C5CE7]' : 'text-slate-400'}`}>
                    {q.id}.
                  </span>
                  <p className="text-slate-700 text-[15px] font-medium leading-relaxed flex-1">
                    {q.text}
                  </p>
                </div>
                    ...
                    {['TRUE', 'FALSE', 'NOT GIVEN'].map((option) => (
                      <label key={option} className="flex items-center gap-3 cursor-pointer group/opt">
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${
                          ...
                          {option === 'FALSE' && <div className="w-2 h-2 bg-white rounded-full" />}
                        </div>
                        <span className={`text-sm font-bold ${option === 'FALSE' ? 'text-[#6C5CE7]' : 'text-slate-400'}`}>
                          {option}
                        </span>
                      </label>
                    ))}
      ...
      {isFeedbackOpen && (
        <div className="fixed top-10 bottom-10 right-10 w-[380px] bg-white rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-slate-100 flex flex-col z-40 animate-in slide-in-from-right-10 duration-500">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div className="bg-slate-50/50 rounded-3xl p-6 text-slate-600 text-[14px] leading-relaxed border border-slate-100">
              <p>Your answer "500" is incorrect because the audio script clearly states that the Empress package costs £450 per person. The spa employee explicitly mentions this price during the conversation. Therefore, "500" does not match the information provided in the script.</p>
            </div>
          </div>

          <div className="p-6 space-y-3 mt-auto">
            <button className="w-full py-3.5 border-2 border-[#8B7EFF] text-[#8B7EFF] font-bold rounded-2xl text-sm hover:bg-slate-50 transition-all">
              Feedback
            </button>
            <button className="w-full py-3.5 bg-[#E8E4FF] text-[#6C5CE7] font-bold rounded-2xl text-sm hover:bg-[#DCD5FF] transition-all">
              Explanation
            </button>
            <button className="w-full py-3.5 border border-slate-200 text-slate-500 font-bold rounded-2xl text-sm hover:bg-slate-50 transition-all">
