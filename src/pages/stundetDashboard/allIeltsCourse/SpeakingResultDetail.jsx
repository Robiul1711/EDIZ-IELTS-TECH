...
const SpeakingResultDetail = () => {
  ...
  const currentResult =
    results.find((r) => r.serial_number === currentSerial) || results[0];

  useEffect(() => {
    if (
      results.length > 0 &&
      !results.find((r) => r.serial_number === currentSerial)
    ) {
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col md:flex-row items-center gap-6">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => {
                const currentIndex = results.findIndex(
                  (r) => r.serial_number === currentSerial,
                );
                if (currentIndex > 0)
                  setCurrentSerial(results[currentIndex - 1].serial_number);
              }}
            ...
            <button
              ...
              disabled={
                currentSerial === results[results.length - 1]?.serial_number
              }
            ...
            <div className="lg:col-span-1 space-y-6">
              <div>
                <h3 className="text-slate-500 font-bold mb-2 uppercase tracking-wider text-sm">
                  Q{currentResult.serial_number}
                </h3>
                <h2 className="text-xl font-bold text-slate-800 dark:text-white leading-relaxed">
                  {currentResult.question_text || "Speaking Question"}{" "}
                  {/* If API adds question text */}
                </h2>
              </div>
            ...
            <div className="lg:col-span-1 space-y-6 lg:border-l lg:border-r border-slate-200 dark:border-slate-800 lg:px-6">
              <div className="bg-slate-50 dark:bg-slate-800/30 rounded-2xl p-6">
                <div className="text-4xl font-black text-slate-800 dark:text-white">
                  {currentResult.score ? currentResult.score.toFixed(1) : "0.0"}{" "}
