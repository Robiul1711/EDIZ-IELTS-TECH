...
const TestSection = () => {
  ...
  const testCards = [
    { label: "Reading", icon: ReadingLogo, bgColor: "bg-[#B4A7FF]", textColor: "text-[#4A3AFF]" },
    { label: "Listening", icon: ListeningLogo, bgColor: "bg-[#FFC978]", textColor: "text-[#92400E]" },
    { label: "Writing", icon: WritingLogo, bgColor: "bg-[#D9F481]", textColor: "text-[#3F6212]" },
    { label: "Speaking", icon: SpeakingLogo, bgColor: "bg-[#C4F2F1]", textColor: "text-[#155E75]" },
  ];

  return (
    <div className="flex flex-col gap-8 w-full">
      {sections.map((section) => (
        <div key={section.key} className="bg-white/50 backdrop-blur-sm rounded-[2.5rem] p-6 md:p-10 border border-white shadow-sm">
          <h3 className="text-base font-bold text-slate-800 mb-8 uppercase tracking-widest">
            {section.title}
          </h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xlg:grid-cols-4 gap-6">
            {testCards.map((card, index) => (
              ...
              >
                <h4 className={`${card.textColor} text-2xl font-black tracking-tighter relative z-10`}>
                  {card.label}
                </h4>
