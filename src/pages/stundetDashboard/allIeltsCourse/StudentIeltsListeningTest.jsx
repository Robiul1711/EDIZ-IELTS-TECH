...
const StudentIeltsListeningTest = () => {
  ...
  const renderQuestionGroup = (group, idx) => {
    return (
      <div key={idx} className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 lg:p-12 shadow-sm border border-slate-100 dark:border-slate-800 transition-all hover:shadow-md">
        ...
        {(() => {
          switch (group.type) {
            case "tfng":
              return <TFNG group={group} answers={answers} onChange={handleAnswerChange} />;
            case "choice":
              return <MCQ group={group} answers={answers} onChange={handleAnswerChange} />;
            case "fill_gap":
              return <FillGap group={group} answers={answers} onChange={handleAnswerChange} />;
            case "matching":
              return <Matching group={group} answers={answers} onChange={handleAnswerChange} />;
            default:
              return (
                <div className="p-10 bg-slate-50 dark:bg-slate-950 rounded-3xl border border-dashed border-slate-200 dark:border-slate-800 flex flex-col items-center text-center">
                    ...
                    <p className="text-slate-500 font-bold text-sm tracking-tight">Question type not supported: {group.type}</p>
                </div>
              );
          }
        })()}
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-slate-50 dark:bg-slate-950 font-sans">
        <div className="relative flex flex-col items-center">
            ...
            <div className="mt-6 text-slate-500 font-black tracking-widest uppercase text-[10px] animate-pulse">Establishing Connection...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-screen bg-white dark:bg-slate-950 overflow-hidden font-sans">
      <TestHeader
        ...
        onExit="/student-dashboard/ielts/listening"
      />
            ...
            <div className="p-8 bg-slate-900 dark:bg-slate-800 rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
              ...
              <div className="relative z-10 space-y-6">
                <div className="flex items-center justify-between">
                    <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">Audio Track</span>
                ...
                <div className="flex items-center justify-center gap-8">
                    <button onClick={() => audioRef.current && (audioRef.current.currentTime -= 10)} className="text-white/40 hover:text-white transition-colors"><RotateCcw size={22} /></button>
                    <button 
                        onClick={togglePlay}
                    ...
                    <button onClick={() => audioRef.current && (audioRef.current.currentTime += 10)} className="text-white/40 hover:text-white transition-colors rotate-180"><RotateCcw size={22} /></button>
                </div>
              </div>
        ...
        <div className="flex-1 h-full overflow-y-auto p-6 lg:p-12 bg-slate-50 dark:bg-slate-950 custom-scrollbar pb-40">
          <div className="max-w-4xl mx-auto space-y-10">
             ...
             <div className="space-y-10">
               {questionGroups.map((group, idx) => renderQuestionGroup(group, idx))}
             </div>
          </div>
        </div>
      </main>
      ...
      <footer className="h-24 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex items-center px-6 lg:px-12 z-20 shadow-[0_-10px_40px_rgba(0,0,0,0.02)]">
        <div className="flex items-center gap-4">
           <div className="flex items-center gap-3 px-5 py-2.5 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700">
             ...
             <span className="text-xs font-black text-slate-700 dark:text-slate-200 font-mono tracking-widest uppercase">Listening SEC {part_no} Active</span>
           </div>
        </div>
        ...
        >
          <span>{isSubmitting ? "Submitting Request..." : isSubmitted ? "Success - Submitted" : "Finalize Test Part"}</span>
