...
const ListeningHeadSetCheck = () => {
    return (
        <div className='flex flex-col min-h-screen items-center gap-6 md:gap-10 px-4 pb-10'>
            ...
            <audio src={ImageAssets.audio} controls className='w-full max-w-md'></audio>

            {/* Adjusted font size and spacing for mobile readability */}
            <ul className="list-decimal space-y-3 mt-4 text-base md:text-lg font-semibold max-w-2xl">
                <li>Put on your headset and click the play button.</li>
                <li>You will hear a short recording.</li>
                <li>
                    If you cannot hear the sound clearly, make sure to check your headset and see if they are working properly.
                </li>
            ...
            <div className="flex flex-col-reverse md:flex-row gap-4 w-full md:w-auto mt-auto md:mt-0">
                <button className='bg-black text-white w-full md:w-auto px-8 py-4 text-lg shadow-lg rounded-lg font-semibold flex items-center justify-center gap-2'>
                    <File size={20} /> Save & Exit
                </button>
                <Link to="/pte/listening/microphone-check">
            ...
            <p className="">In the actual test, the play button will be deactivated. The audio recording will be played automatically.</p>
        </div>
    );
};
