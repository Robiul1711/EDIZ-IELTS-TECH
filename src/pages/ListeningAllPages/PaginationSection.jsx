...
const PaginationSection = ({ activePart = 1, onPartChange }) => {
  ...
  return (
    <div className="bg-white rounded-xl section-padding-x shadow-sm border border-gray-100 p-6 sticky bottom-0 w-full">
      <div className="flex justify-between gap-8 max-w-7xl mx-auto text-sm">
        ...
        <div className="flex items-center gap-3 justify-center">
          <button 
            ...
            className={`w-10 h-10 flex items-center justify-center rounded-full transition-colors text-gray-500 
              ${activePart === 1 ? 'bg-gray-50 opacity-50 cursor-not-allowed' : 'bg-gray-100 hover:bg-gray-200'}`}
          >
            <ChevronLeft size={20} />
          </button>
          ...
          <button 
            ...
            className={`w-10 h-10 flex items-center justify-center rounded-full transition-colors text-white shadow-md shadow-purple-200
              ${activePart === 4 ? 'bg-purple-300 cursor-not-allowed' : 'bg-custom hover:bg-purple-600'}`}
          >
...
const PaginationGroup = ({ label, total, partNumber, isActivePart, onClick }) => {
  ...
  return (
    <div 
      onClick={onClick}
      className={`flex items-center justify-center gap-3 cursor-pointer transition-opacity ${isActivePart ? 'opacity-100' : 'opacity-60 hover:opacity-100'}`}
    >
      <span className={`font-semibold ${isActivePart ? 'text-purple-700' : 'text-slate-800'}`}>
        {label}:
      </span>
